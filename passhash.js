...
/* passhash v2 by Benji Matta
* modifications of original:
* 1) added minimist to allow passing arguments from command line (see details for valid arguments below)
*    what is minimist? - an argument parser that allows multiple arguments passed in the command line to be combined and passed in any order
* 	eg. -dpm will set all of requireDigit, requirePunctuation and requireMixedCase to 'true'
* 2) will copy resulting password to clipboard
* 
*	Required argument
*	- the only required argument is siteTag
*	- when used alone, defaults for masterKey and hashWordSize are set as noted below
*	
*	Optional arguments to be passed to passhash2.js 
* - argument names that have more than one character must be preceeded by --
* - argument bump will append :[number] to the siteTag (eg, amazon -b3 will set siteTag to 'amazon:3')
* - summary of arguments [* indicates required, {alias} represents alternative parameter name, default parameter values in ()]:
* 	*siteTag
* 	masterKey {key} (default) [note: must follow siteTag if set to non-default value]
*	hashWordSize {l} (8)
*	requireDigit {d} (false)
*	requirePunctuation {p} (false)
*	requireMixedCase {m} (false)
*	restrictSpecial {s} (false) [no special characters will be allowed]
*	restrictDigits {D} (false)
*	bump {b}
*
...
boolean: ['requireDigit','requirePunctuation','requireMixedCase','restrictSpecial','restrictDigits'],
alias: {d: 'requireDigit', p: 'requirePunctuation', m: 'requireMixedCase', s: 'restrictSpecial', D: 'restrictDigits', key: 'masterKey', l: 'hashWordSize',b:'bump'},
default: { masterKey: 'default', siteTag: '', hashWordSize: 8, requireDigit: false, requirePunctuation: false, requireMixedCase: false, restrictSpecial: false, restrictDigits: false}
})
args.siteTag=args._[0];
...
var PassHashCommon =
{
	...
	if (typeof bump !== 'undefined') siteTag = siteTag.concat(':',bump);
	if (typeof args._[1] !== 'undefined') masterKey = args._[1];
        var s = sha1(masterKey,siteTag);
...
//console.log(args);
console.log(PassHashCommon.generateHashWord(args.siteTag,args.masterKey,args.hashWordSize,args.requireDigit,args.requirePunctuation,args.requireMixedCase,args.restrictSpecial,args.restrictDigits,args.bump));
